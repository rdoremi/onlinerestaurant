...
@JsonSerialize(include = JsonSerialize.Inclusion.NON_DEFAULT )//保证序列化json的时候，如果是null的对象，key也会消失
public class ServerResponse<T> implements Serializable {
    ...
    public static <T> ServerResponse<T> createBySuccess(T data){
        return new ServerResponse<>(ResponseCode.SUCCESS.getCode(),data);//private ServerResponse(int status,T data){
    }
    public static <T> ServerResponse<T> createBySuccess(String msg,T data){
        return new ServerResponse<>(ResponseCode.SUCCESS.getCode(),msg,data);//ServerResponse(int status,String msg,T data)
    }
